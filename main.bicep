...
resource uploadDeploymentScript 'Microsoft.Resources/deploymentScripts@2020-10-01' = {
  ...
  properties: {
    ...
    scriptContent: '''
      ...
      wget -O $FILE_NAME https://community.opengroup.org/osdu/platform/data-flow/data-loading/open-test-data/-/archive/M10/open-test-data-M10.tar.gz 2>&1 | tee -a $LOG
      ...
      tar -xzvf $FILE_NAME -C $DATA_DIR/datasets/documents --strip-components=5 open-test-data-M10/rc--1.0.0/1-data/3-provided/USGS_docs
      echo -e "Extracted Dataset Documents" 2>&1 | tee -a $LOG

      tar -xzvf $FILE_NAME -C $DATA_DIR/datasets/markers --strip-components=5 open-test-data-M10/rc--1.0.0/1-data/3-provided/markers
      echo -e "Extracted Dataset Markers" 2>&1 | tee -a $LOG

      tar -xzvf $FILE_NAME -C $DATA_DIR/datasets/trajectories --strip-components=5 open-test-data-M10/rc--1.0.0/1-data/3-provided/trajectories
      echo -e "Extracted Dataset Trajectories" 2>&1 | tee -a $LOG

      tar -xzvf $FILE_NAME -C $DATA_DIR/datasets/well-logs --strip-components=5 open-test-data-M10/rc--1.0.0/1-data/3-provided/well-logs   
      echo -e "Extracted Dataset Well Logs" 2>&1 | tee -a $LOG

      tar -xzvf $FILE_NAME -C $DATA_DIR/schema --strip-components=3 open-test-data-M10/rc--3.0.0/3-schema
      echo -e "Extracted Schemas" 2>&1 | tee -a $LOG
      
      tar -xzvf $FILE_NAME -C $DATA_DIR/templates --strip-components=3 open-test-data-M10/rc--3.0.0/5-templates
      echo -e "Extracted Templates" 2>&1 | tee -a $LOG

      tar -xzvf $FILE_NAME -C $DATA_DIR/TNO/contrib --strip-components=5 open-test-data-M10/rc--3.0.0/1-data/3-provided/TNO
      echo -e "Extracted TNO Contrib" 2>&1 | tee -a $LOG

      tar -xzvf $FILE_NAME -C $DATA_DIR/TNO/provided --strip-components=3 open-test-data-M10/rc--3.0.0/4-instances/TNO
